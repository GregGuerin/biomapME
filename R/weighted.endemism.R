		...
		if(weight.type=="geo") {
		  ...
		  #####################################
			CalcDists <- function(longlats) { #modified from CalcDists.R, see https://gist.githubusercontent.com/sckott/931445/raw/9db1d432b2308a8861f6425f38aaabbce44eb994/CalcDists.R
				...
				for (i in 1:n) {
					for (j in 1:n) z[i, j] <- geosphere::distCosine(c(longlats[j, 1], longlats[j, 2]), c(longlats[i, 1], longlats[i, 2]))
					}
				z <- as.dist(z)
				return(z)
				} #cls CalcDists
				...
				species_ranges <- function(x) {
					...
					for (i in 1:ncol(x)) {
						...
						if(any(temp$species_i == 0)) {
							...
							sp::coordinates(temp) <- c("LONGITUDE", "LATITUDE") #xy of occupied cell centroids
							...
							if(geo.calc == "polygon") {
								...
								if(nrow(as.data.frame(temp)) < 5) {
									polygon_area <- try(geosphere::areaPolygon(as.data.frame(temp)))
					...
					for (i in colnames(cell_occur_matrix)) {
						...
						temp <- x[which(x$SPECIES == i),]
								...
								if(geo.calc == "polygon") {
									...
									if(nrow(as.data.frame(temp)) < 5) {
										polygon_area <- try(geosphere::areaPolygon(as.data.frame(temp)[,c("LONGITUDE", "LATITUDE")]))
										if(class(polygon_area) == "try-error") {
											v[n] <- cell_size
